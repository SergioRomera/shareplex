--#####################################################################################################################################
--
-- Script Name:        	Cust_DBInfo_v82.sql
...
-- When:                       January 2016
-- Who:			       Clay Jackson
-- What:   1. Added section to report on datatypes not supported for replication to PostgreSQL for use with SharePlex 8.6.3
-- 	      2. Added newly supported data types for SQLServer (Timezone, NCHAR, NVARCHAR)					
...
set heading off;
prompt *********************************************************************
prompt ****   	List Data Types Not Supported for Replication to        ****
prompt ****                    SQLServer                                ****
prompt *********************************************************************
...
where ((data_type in ('ROWID','BFILE','XMLTYPE','VARRAY','SDO_GEOMETRY')) or
data_type like 'INTERVAL%' or
data_type like 'TIMESTAMP%WITH%LOCAL%' or
data_type like 'BINARY%' ) and
a.owner not in &user_exclusion
and a.table_name = b.object_name
and b.object_type = 'TABLE'
order by a.owner;
...
prompt *********************************************************************
prompt ****   	List Data Types Not Supported for Replication to        ****
prompt ****                    Postgres Plus Advanced Server            ****
prompt *********************************************************************
timing start NoRepPPAS;
...
data_type like 'TIMESTAMP%WITH%' ) and
a.owner not in &user_exclusion
and a.table_name = b.object_name
and b.object_type = 'TABLE'
order by a.owner;

timing stop NoRepPPAS;
